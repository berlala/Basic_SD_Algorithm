function [a ,s, memory] = controller_mpc_steering_steps(x_car,y_car,psi,v,alpha,memory,Goal_inf, N)
...
r = [x_goal, y_goal,  theta_goal, v_goal, alpha_goal]'; % original point as the target point
       ...
       R = diag([Rv Rs]);
       P =diag([5 5 5 1 1]); % final step cost
       ...
       if abs(x_car - x_goal) < 5 && abs(y_car - y_goal) < 0.2 % apporach the destination
           ...
           Q = diag([Qx Qy Qpsi Qv Qalpha]);
           R = diag([Rv Rs]);
          P =diag([0 0 0 2 2]);  % final step cost
       end
...
Xmax = [50, 5.625 , 2*pi, 7, 45/180*pi]'; % state constrain, X, Y, heading, velocity, Front Wheel Angle
