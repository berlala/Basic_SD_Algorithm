function [a ,alpha, memory] = controller_mpc_Tunner(x_car,y_car,psi,v,memory,Goal_inf, N)
...
Xmax = [55, 5.625 , 2*pi, 7]'; % state constrain, X, Y, heading, velocity
Xmin = [0,-1.875,  -2*pi, 0.1]';
%umax = [5-v0,   0.6-alpha0]'; % maximum constrain on input
%umin = [-3-v0,    -0.6-alpha0]'; % minimum constain on input
umax = [4*Ts,   45/180*pi*Ts]'; % maximum constrain on input, should take time simple into account
umin = [-8*Ts,  - 45/180*pi*Ts]'; % minimum constain on input
