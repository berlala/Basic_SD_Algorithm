...
function [  ] = PlotLog( X,U,Y,track,track2,simN,Ts)
    ...
    %plot(X(1,:),X(2,:),'b') % original plot 
     for i=1:length(U(3,:))  % updated plot , display the velocity by different color
     plot(X(1,i),X(2,i),'.','MarkerSize',5,'color',[U(3,i)/max(U(3,:)) 0.3 U(3,i)/max(U(3,:))]);
     hold on; 
     end
