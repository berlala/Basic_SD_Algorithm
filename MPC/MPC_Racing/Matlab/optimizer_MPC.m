...
function [x, u, new_b, exitflag,info] = optimizer_MPC(TrackMPC,MPC_vars,ModelParams,n_cars, Y, x, u,  x0, u0)
    ...
    if (~isempty(info.exitflag.problem))
        %chaneg the condition from [info.exitflag ==0]
        exitflag = 0;
        x = xpred;
        u = upred;
    else
