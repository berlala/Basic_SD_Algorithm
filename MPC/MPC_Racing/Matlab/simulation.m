...
%track includes left,right and center coordinate
...
if CarModel ==  'ORCA' 
    vx0 = 1;
elseif CarModel == 'FullSize' %#ok<*STCMP>
    vx0 = 15;
end
...
[theta, ~] = findTheta([track.center(1,startIdx),track.center(2,startIdx)],...
    track.center,traj.ppx.breaks,trackWidth,startIdx);

%initial state , dppx,dppy
...
% Note by Bolin:
% the state include [point on centerline X, point on centerline Y, centerline point heading��
%                                 vehicle spd, ? ,? theta]
...
for i = 1:2 % origin try 5 times (repeat)
    % formulate MPCC problem and solve it
    Iter_damping = 0.5; % 0 no damping
    [x_up, u_up, b, exitflag,info] = optimizer_MPC(TrackMPC,MPC_vars,ModelParams, ...
        n_cars, Y, x, u, x0, uprev);
    ...
    if plotOn == 1
        ...
        %x(1,:),x(2,:) are the path for the prediction 
    end
end
%% Simulation
for i = 1: simN   
    ...
    %  formulate MPCC problem and solve it
    if QP_iter == 0
        [x, u, b, exitflag,info] = optimizer_MPC(TrackMPC,MPC_vars,ModelParams, ...
            n_cars, Y, x, u, x0, uprev);
