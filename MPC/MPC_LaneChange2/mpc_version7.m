...
% To-Do: add the difference on two sample acceleartion into cost
...
Horizon =10;
X00 = [-0.5 0 0.1 -pi/2];% Initial Status
...
for i=1:1:N
   psi =X00(4); %ref psi
    ...
    A_cons=[];
    b_cons=[];
    lb= [-10;-540/180*pi]; %input lower boundary
    ub=[10; 540/180*pi]; %input upper boundary
    
    tic
    [U_e,fval(i,1),exitflag(i,1),output(i,1)]=quadprog(G,f,A_cons,b_cons,[],[],lb,ub);% qp solver
    toc
