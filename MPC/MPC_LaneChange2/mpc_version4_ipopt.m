...
for i=1:1:Nr
    ...
    lb_apm =repmat(lb,Horizon,1);
    ub_apm = repmat(ub,Horizon,1);
    
    tic
    %[U_e,fval(i,1),exitflag(i,1),output(i,1)]=quadprog(H,f,A_cons,b_cons,[],[],lb,ub,x0);% qp solver

     y =apm_quadprog(H,f,A_cons,b_cons,[],[],lb_apm,ub_apm,x0);% apm-ipopt solver
    U_e_2 = [y.values]';
    toc
    
    X_error(i,:)=(A*x_error(i,:)'+B*U_e_2)';
    ...
    u_error(i,1)=U_e_2(1,1);
    u_error(i,2)=U_e_2(2,1);
