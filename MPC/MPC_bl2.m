function [delta,e, th_e,ind,tgt,CAL] = MPC_bl2(initial_ENH,v, RouteMapArray, CurvDerVehMap, pe, pth_e,Preview,delta_f) 
...
Xs = zeros(4, 1);%# Ref: e[y v r theta]
Xs(1, 1) =  e;   
Xs(2, 1) = (e - pe) / dt;
Xs(3, 1) = (th_e - pth_e) / dt;
Xs(4, 1) =  th_e;
...
A1 = -(2*Cf+2*Cr)/(m*v);
B1 = -(2*Cf*lf-2*Cr*lf)/(m*v)-v;
A2 = -(2*Cf*lf-2*Cr*lr)/(Iz*v);
B2 = -(2*Cf*lf^2+2*Cr*lr^2)/(Iz*v);
C1 = (2*Cf)/m;
C2 = (2*Cf*lf)/Iz;
...
Q(3,3) =0;
Q(4,4) =0;
R = 1; % increase R to reduce the flucation of the cmd
N =10; % horizon
...
for k = 1:N
    ...
    constraints = [constraints, x{k+1} == Ad*x{k}+Bd*u{k}]; % A discrete model
