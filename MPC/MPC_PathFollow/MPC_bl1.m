function [delta,e, th_e,ind,tgt,CAL] = MPC_bl1(initial_ENH,v, RouteMapArray, CurvDerVehMap, pe, pth_e,Preview,delta_f) 

yalmip('clear')
% Ref: https://yalmip.github.io/example/standardmpc
CAL = {};
% Change Log:
% 2018.12.7: change the reference from 0 to [e,th_e]
% change Q,R to current value 
...
th_e =(initial_ENH(3)- RouteMapArray(tgt,3)); % Heading error [rad]
...
nu = 1; % Number of inputs
 ...
 Ad = sysd.a;
 Bd = sysd.b;
 Kd = dt*[0, -(2*Cf*lf-2*Cr*lf)/(m*v)-v, 0, -(2*Cf*lf^2+2*Cr*lr^2)/(Iz*v)]';
 %Kd = syss.d;
...
Q(3,3) =0.002;
Q(4,4) =0;
R = 14; % increase R to reduce the flucation of the cmd
P= 2*Q;

N =10; % horizon

ny = 4; %observer
C = [1 0 1 0]; % only two states can be got

u = sdpvar(repmat(nu,1,N),repmat(1,1,N));
x = sdpvar(repmat(nx,1,N+1),repmat(1,1,N+1));
r = sdpvar(repmat(ny,1,N+1),repmat(1,1,N+1)); % ny*1*n
...
for k = 1:N
    ...
    constraints = [constraints, x{k+1} == Ad*x{k}+Bd*u{k}+  Kd*Theta{k} +E*d]; % A discrete model
    constraints = [constraints, -45/180*pi<= u{k}<= 45/180*pi, [-5,-10, -pi/3, -pi]'<=x{k+1}<=[5,10, pi/3, pi]'];
end
objective = objective + (C*x{N+1}-r{N+1})'*P*(C*x{N+1}-r{N+1}); % final cost

parameters_in = {x{1},[r{:}],[Theta{:}],d,pastu,B}; % input definition
...
Bmodel = Bd;
Reference = zeros(4,N+1); % The target error [TBD]
Reference(1,:) =-e*(1:-1/(N):0);
Reference(3,:) =-th_e*(1:-1/(N):0);
Theta_In = ones(1,N+1) *Theta_dot;
inputs = {Xs,Reference,Theta_In,disturbance,oldu,Bmodel};
