%% Artificial Field Attract/Reject Test
%%
%bolinzhao
%% 
close all;clear
...
Obs = [3,2;
            3,3;
             ...
             8,9];
         R_Num  = 100;
         X_c = 0; % initial position X
         Y_c = 0; % initial position Y
...
for k1 = 1:R_Num
                 ...
                 for k3 = 1:length(Obs)
                     ...
                     F_rre(k3) = a*(1/rre(k3)  - 1/P0 ) *1/(rre(k3)^2);
                     F_ata(k3) = b*rat;
                     
                     if  isinf(Y_rre(k3))==1|| Y_rre(k3)>50
                         Y_rre = 50;
                     end
...
hold on
plot(Obs(:,1)/0.2, Obs(:,2)/0.2, '*','MarkerSize',P0*2)
legend('Path', 'Goal','Obs', 'location','best')
title('Path')

for k5 =1:length(pos_xh)
figure(3)
plot3((pos_xh(k5)-1)*density, (pos_yh(k5)-1)*density, SUM(pos_yh(k5), pos_xh(k5)),'*-r')
grid on
hold on
end
title('Field')
