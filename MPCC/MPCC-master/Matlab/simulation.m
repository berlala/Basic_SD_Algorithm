...
for i = 1:1 % origin try 5 times (repeat)
    % formulate MPCC problem and solve it
    Iter_damping = 0.5; % 0 no damping
    [x_up, u_up, b, exitflag,info] = optimizer_MPC(TrackMPC,MPC_vars,ModelParams, n_cars, Y, x, u, x0, uprev);
...
for i = 1: simN   
    ...
    %  formulate MPCC problem and solve it
    if QP_iter == 0
        [x, u, b, exitflag,info] = optimizer_MPC(TrackMPC,MPC_vars,ModelParams, n_cars, Y, x, u, x0, uprev);
