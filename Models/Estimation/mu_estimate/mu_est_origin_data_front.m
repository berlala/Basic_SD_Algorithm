...
figure(2)
subplot(2,1,1)
plot(Fyf_est);hold on
plot(Fyf_cs);
title('Lateral Force[N]')
subplot(2,1,2)
plot(Fyr_est);hold on
plot(Fyr_cs);
...
%%  e.g., The Front Tire from Carsim Data
 ...
 Fyf_offline_cs = Cf_axle*-alpha_f_cs;
 data_saturation_cs = [];

for i = 1:length(vy)
    if ((abs(Fyf_offline_cs(i)) - deadzone_down)>abs(Fyf_cs(i))) && (abs(Fyf_cs(i))>400) && ((abs(Fyf_offline_cs(i)) <(abs(Fyf_cs(i))+deadzone_up))) && (sign(Fyf_offline_cs(i))==sign(Fyf_cs(i)))
       % disp('Detect Saturation!')
        data_saturation_cs = [data_saturation_cs;[i,alpha_f_cs(i), Fyf_offline_cs(i),Fyf_cs(i)]];
    else
    end  
end
...
% figure(5)
% plot(Fyf_offline_cs); hold on
% plot(Fyf_cs, 'linewidth',1);
...
for i = 2:length(vy)
  if (abs(Fyf_offline_est(i)) - deadzone_down)>abs(Fyf_est(i)) && (abs(Fyf_est(i))>400) && ((abs(Fyf_offline_est(i)) <(abs(Fyf_est(i))+deadzone_up)))  && (sign(Fyf_offline_est(i))==sign(Fyf_est(i)))...
                    &&(abs(Fyf_offline_est(i)) > abs(Fyf_offline_est(i-1))) && (abs(alpha_f_est(i))>3.0/180*pi) 
      %  disp('Detect Saturation!')
        data_saturation_est = [data_saturation_est;[i,alpha_f_est(i), Fyf_offline_est(i), Fyf_est(i)]];%[index, alpha_angle,Fy_offline_est, Fy_realtime_est]
    else
    end  
end
disp('Finish Saturation Detection!')
figure(6)
plot(Fyf_offline_est); hold on
plot(Fyf_est);
